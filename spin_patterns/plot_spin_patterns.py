...
# -*- coding: utf-8 -*-
"""
Created on November 28 14:03 2024
Created in PyCharm
Created as sphenix_pp_qa/plot_spin_patterns
...
def main():
    ...
    # plot_good_runs(spin_db_df)
    plot_bad_runs(spin_db_df)

    print('donzo')


def plot_good_runs(spin_db_df):
    """
    Plot spin patterns for good runs and write unique spin patterns to a file.
    :param spin_db_df:
    :return:
    """
    ...
    write_unique_spin_patterns(spin_db_df)
    plot_spin_patterns(spin_db_df)


def plot_bad_runs(spin_db_df):
    """
    Plot spin patterns for bad runs.
    :param spin_db_df:
    :return:
    """
    phys_runs = spin_db_df[spin_db_df['Type'] == 'physics']
    bad_runs = phys_runs[phys_runs['badrunqa'] == 1]

    plot_spin_patterns(bad_runs)

def write_unique_spin_patterns(spin_db_df):
    """
    Get unique spin patterns for blue and yellow and write them to a file.
    :param spin_db_df:
    :return:
    """
    ...
    with open('blue_spin_patterns.txt', 'w') as file:
        for pattern in blue_fill_patterns:
            file.write(f'{pattern}\n')

    with open('yellow_spin_patterns.txt', 'w') as file:
